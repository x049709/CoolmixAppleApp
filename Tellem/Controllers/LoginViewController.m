...
- (void)viewDidLoad
{
    ...
    if ([[[PFUser currentUser]valueForKey:@"Accounttype"]isEqualToString:@"FB"])
    {
        ...
        if ([[PFUser currentUser]isAuthenticated])
        {
            ...
            [PFFacebookUtils logInWithPermissions:@[ @"user_about_me", @"user_relationships", @"user_birthday", @"user_location",@"publish_actions"] block:^(PFUser *user, NSError *error)
             {
...
- (IBAction)fbSigninTouched:(id)sender
{
    ...
    [PFFacebookUtils logInWithPermissions:@[ @"user_about_me", @"user_relationships", @"user_birthday", @"user_location",@"publish_actions",
             ...
             [[FBRequest requestForMe] startWithCompletionHandler:
              ^(FBRequestConnection *connection, NSDictionary<FBGraphUser> *user, NSError *error)
              {
                  if (!error)
                  {
                      ...
                      [PFUser logInWithUsernameInBackground:[user valueForKey:@"id"] password:[user valueForKey:@"id"]
                                                      block:^(PFUser *user, NSError *error)
                       {
                           ...
                           [usernew setObject:imageFile1 forKey:@"profilePictureMedium"];
        ...
        if ([[UrlParts objectAtIndex:(1)] isEqualToString:@"NeverGonnaFindMe"])
        {
            ...
            if(urlParamatersArray.count==1)
            {
                ...
                if (str_Accesstoken.length>0)
                {
                    ...
                    [PFUser logInWithUsernameInBackground:instagramId password:instagramId
                                                    block:^(PFUser *user, NSError *error)
                     {
                         if(error)
                         {
...
-(void)getTwitterUserInfo
{
    ...
    [self logTimeline:twResponseArray];
}

- (void)logTimeline: (NSArray *) userArray
{
    [self.view.window addSubview:ApplicationDelegate.hudd];
    [ApplicationDelegate.hudd show:YES];
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
        ...
        @autoreleasepool {
            dispatch_sync(dispatch_get_main_queue(), ^{
                @autoreleasepool {
                    ...
                    NSString *name = [userArray valueForKey:@"name"];
                    NSString *twitter_Int;
                    twitter_Int=[userArray valueForKey:@"id"];
                    ...
                    if([[PFUser currentUser]isAuthenticated])
                    {
                        ...
                        user.password=@"123";
                        NSString *profile_image_url_https = [userArray valueForKey:@"profile_image_url_https"];
                        ...
                        {
                            [ApplicationDelegate.hudd hide:YES];
                            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"Tellem" message:@"Error logging in to Twitter" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
                            [alert show];
                        }
                        }];
                    }
                        ...
                        [PFUser logInWithUsernameInBackground:twitter_Id password:@"123"
                                                        block:^(PFUser *user, NSError *error)
                         {
                             if(error)
                             {
                                 ...
                                 [user setObject:@"Twitter" forKey:@"Accounttype"];
                                 [user setObject:[userArray valueForKey:@"name"] forKey:@"displayName"];
                                 NSData *newdata=[NSData dataWithContentsOfURL:[NSURL URLWithString:[userArray valueForKey:@"profile_image_url_https"]]];
                                 NSString *imgname1 = [ApplicationDelegate findUniqueSavePath];
                                 PFFile *imageFile1 = [PFFile fileWithName:imgname1 data:newdata];
                                 [user setObject:imageFile1 forKey:@"profilePictureMedium"];
                                 if (user[@"ShareSettings"]==nil) {
                                 ...
                                 [user signUpInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
                                  {
                                      ...
                                      {
                                          [ApplicationDelegate.hudd hide:YES];
                                          UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"Tellem" message:@"Error occured logging in to Twitter" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
                                          [alert show];
                                      }
